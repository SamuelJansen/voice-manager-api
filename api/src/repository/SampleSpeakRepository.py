...
@Repository(model = SampleSpeak.SampleSpeak)
class SampleSpeakRepository:
    ...
    def existsByAccountKey(self, accountKey) :
        exists = self.repository.session.query(sap.exists().where(self.model.accountKey == accountKey)).one()[0]
        self.repository.session.commit()
        return exists

    def findByAccountKey(self, accountKey):
        if self.existsByAccountKey(accountKey) :
            model = self.repository.session.query(self.model).filter(self.model.accountKey == accountKey).first()
            self.repository.session.commit()
            return model

    def notExistsByAccountKey(self, accountKey) :
        return not self.existsByAccountKey(accountKey)
